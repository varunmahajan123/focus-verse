  ...
  return (
    <section className="app-section">
      ...
      <div className="grid gap-5 lg:grid-cols-[0.92fr_1.08fr]">
        <AppCard className="bg-ink text-white">
          <div className="flex items-center gap-3">
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-coral text-ink">
              <AlertTriangle size={22} />
            </span>
            <div>
              <p className="text-sm font-black text-white/45">Recovery summary</p>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            <div className="rounded-[1.2rem] bg-white/10 p-4">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-white/38">Hours</p>
              <p className="mt-3 text-3xl font-black">{backlog.estimatedHours}h</p>
            </div>
            <div className="rounded-[1.2rem] bg-white/10 p-4">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-white/38">Category</p>
              <p className="mt-3 text-2xl font-black">{backlog.mostDelayedCategory}</p>
            </div>
            <div className="rounded-[1.2rem] bg-white/10 p-4">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-white/38">Daily target</p>
        ...
        <AppCard>
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-black text-ink/45">Recovery roadmap</p>
          ...
          {backlog.count ? (
            <div className="space-y-3">
              {["Audit what is late", "Compress into 25–45 minute blocks", "Place one recovery block daily", "Review and remove guilt debt"].map((step, index) => (
                <div key={step} className="flex items-center gap-3 rounded-[1.2rem] bg-cream/75 p-4">
                  ...
                  {index < 3 ? <ArrowRight className="ml-auto hidden text-ink/30 md:block" size={18} /> : null}
                </div>
              ))}
              <div className="mt-5 rounded-[1.2rem] bg-mist/65 p-4 text-sm font-bold leading-6 text-ink/58">
