...
const typeTone: Record<PlannerBlock["type"], string> = {
  "deep-work": "bg-mist text-foreground",
  pomodoro: "bg-lavender text-foreground",
  break: "bg-sage text-foreground",
  lunch: "bg-coral/50 text-foreground",
  review: "bg-muted text-foreground",
  shutdown: "bg-foreground text-background",
  recovery: "bg-coral/60 text-foreground"
};
    ...
    <section className="app-section">
      ...
      <div className="grid gap-5 lg:grid-cols-[0.32fr_0.68fr]">
        <AppCard className="h-fit">
          ...
          <p className="mt-2 text-sm font-bold text-muted-foreground">time blocks today</p>
          <div className="mt-6 h-3 overflow-hidden rounded-full bg-border">
            <div className="h-full rounded-full bg-foreground" style={{ width: `${blocks.length ? (completed / blocks.length) * 100 : 0}%` }} />
          </div>
          <p className="mt-3 text-sm font-black text-muted-foreground">{completed}/{blocks.length || 0} complete</p>
        </AppCard>

        <AppCard>
          {blocks.length ? (
            <div className="relative space-y-3 before:absolute before:bottom-6 before:left-[5.15rem] before:top-6 before:w-px before:bg-border max-sm:before:left-[4.25rem]">
              {blocks.map((block) => (
                <div key={block.id} className="relative grid grid-cols-[4.6rem_1fr] gap-4 rounded-[1.25rem] bg-surface-elevated p-3 shadow-sm sm:grid-cols-[5.5rem_1fr_auto] sm:p-4">
                  <div className="text-sm font-black leading-6 text-muted-foreground">
                    <div>{block.start}</div>
                    <div>{block.end}</div>
                  </div>
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {block.completed ? <span className="rounded-full bg-sage text-foreground px-3 py-1 text-xs font-black">Done</span> : null}
                    </div>
